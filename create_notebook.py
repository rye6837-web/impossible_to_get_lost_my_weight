    ...
    "cells": [
        {
            ...
            "source": [
                ...
                "\n",
                "> **목적**: 7차시 `Project_Example.ipynb`와 동일한 방식으로, 식약처 DB 검색 ➔ METs 운동 계산 ➔ 영양 RAG ➔ 5대 Flash 모델 자동 폴백 ➔ Self-RAG 품질 검증 ➔ SQLite DB 연동까지 전 과정을 단계별로 직접 실행하고 검증합니다.\n",
                "\n",
                "**주요 검증 항목:**\n",
                "1. **환경 설정 & API 키 로드**\n",
                "2. **[Tool 1] 식약처 표준 영양 CSV DB 검색기 (`search_food_nutrition`)**\n",
                "3. **[Tool 2] ACSM 표준 METs 운동 소모 칼로리 계산기 (`calculate_exercise_calories`)**\n",
                "4. **[Tool 3] 다이어트 & 임상 영양 백과 RAG 검색기 (`search_nutrition_knowledge`)**\n",
                "5. **[AI Agent] 다중 Flash 모델 무중단 폴백(Fallback) & 3대 Tool 바인딩**\n",
                "6. **[Human-in-the-Loop] 구조화된 메타데이터 태그 자동 파싱 (`parse_agent_metadata`)**\n",
                "7. **[Self-RAG] 3단계 품질 게이트 (관련성 ➔ 환각 검출 ➔ 임상 가드레일)**\n",
                "8. **[DB & 순 칼로리] SQLite 연동 및 일별 순 칼로리(Net Calories) 집계**"
            ]
        },
        {
            "cell_type": "markdown",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 2. [Tool 1 검증] 식약처 표준 영양 DB 검색 (`search_food_nutrition`)\n",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 3. [Tool 2 검증] ACSM 표준 METs 운동 소모 칼로리 계산 (`calculate_exercise_calories`)\n",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 4. [Tool 3 검증] 다이어트 & 임상 영양 백과 RAG 검색 (`search_nutrition_knowledge`)\n",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 5. [AI Agent & 5대 Flash 모델 폴백] DietAgent 클래스 및 도구 바인딩\n",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 6. [시나리오 1 테스트] 식단 분석 및 메타데이터 태그 (`<!-- MEAL_DATA -->`) 자동 파싱 검증"
            ]
        },
        {
            "cell_type": "code",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 7. [시나리오 2 테스트] 운동 기록 및 메타데이터 태그 (`<!-- EXERCISE_DATA -->`) 자동 파싱 검증"
            ]
        },
        {
            "cell_type": "code",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 8. [Self-RAG 3단계 품질 게이트 검증] LLM-as-a-Judge 평가\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "    # DB의 칼로리와 추출된 메타데이터 칼로리의 정합성 검증\n",
                "    g2_grounded = True\n",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## 9. [SQLite DB & 순 칼로리 집계 검증] `app_db/database.py`\n",
...
output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Diet_Agent_Pipeline_Verification.ipynb")
with open(output_path, "w", encoding="utf-8") as f:
    json.dump(notebook, f, indent=2, ensure_ascii=False)

print(f"✅ 주피터 노트북 파일 생성 완료: {output_path}")
