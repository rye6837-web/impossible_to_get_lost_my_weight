...
# 1. 경로 설정: 프로젝트 루트, agent, tools, db 폴더를 파이썬 검색 경로에 등록
...
if st.session_state.user is None:
    st.title("🥗 AI 다이어트 & 영양 코칭 서비스")
    st.caption("개인 맞춤형 식단 분석, 데이터베이스 통계 및 웰니스 코칭 플랫폼")
...
# 사이드바: 내 프로필, 목표 현황, 식단 간편 등록
with st.sidebar:
    st.markdown(f"### 👤 **{current_user['username']}** 님")
    st.caption(f"신체: {current_user.get('gender', '남성')} | {current_user.get('height', 175)}cm | {current_user.get('weight', 70)}kg")
    ...
    st.markdown("#### 📅 오늘 섭취 현황")
    st.metric(
        label="오늘 칼로리", 
        value=f"{today_sum['total_cal']} kcal", 
        delta=f"{round(today_sum['total_cal'] - target_cal, 1)} kcal (목표: {target_cal})",
        delta_color="inverse" if today_sum['total_cal'] > target_cal else "normal"
    )
    st.metric(
        label="오늘 단백질", 
        value=f"{today_sum['total_protein']} g", 
        delta=f"{round(today_sum['total_protein'] - target_protein, 1)} g (목표: {target_protein})"
    )
    
    st.divider()
    st.markdown("#### 🍱 식단 직접 기록하기")
    with st.expander("➕ 새 식단 기록 추가", expanded=False):
        with st.form("manual_meal_form"):
            f_date = st.date_input("식사 날짜", value=date.today())
            ...
            f_memo = st.text_input("메모 / 코칭 피드백", placeholder="특이사항 입력")
            save_btn = st.form_submit_button("DB에 저장", use_container_width=True, type="primary")
...
with tab_coach:
    ...
    st.caption("식품의약품안전처 표준 영양 DB 기반 맞춤형 식단 분석 및 실시간 피드백")
    ...
    if st.session_state.agent is None:
        ...
        st.info("💡 Tip: `.streamlit/secrets.toml` 파일에 `GEMINI_API_KEY = '...'`를 등록해두시면 매번 입력하지 않아도 자동 로드됩니다.")
...
# ==========================================
# 2. 식단 통계 대시보드 탭 (일/주/월/년)
# ==========================================
with tab_dashboard:
    st.header("📊 개인 맞춤 식단 통계 대시보드")
    ...
    with subtab_daily:
        ...
        m2.metric("단백질 섭취량", f"{daily_res['total_protein']} g", f"목표 {target_protein} g")
        m3.metric("탄수화물", f"{daily_res['total_carbs']} g")
        m4.metric("지방", f"{daily_res['total_fat']} g")
        ...
        with c_chart1:
            # 칼로리 게이지 차트
            cal_percent = min(200, (daily_res['total_cal'] / target_cal * 100)) if target_cal > 0 else 0
            fig_gauge = go.Figure(go.Indicator(
                mode="gauge+number+delta",
                value=daily_res['total_cal'],
                title={'text': f"<b>{date_str} 칼로리 달성도</b>"},
                delta={'reference': target_cal, 'increasing': {'color': "#EF553B" if daily_res['total_cal'] > target_cal else "#00CC96"}},
                gauge={
        ...
        if daily_res['records']:
            for rec in daily_res['records']:
                with st.container():
                    ...
                    with c_r1:
                        st.markdown(f"**[{rec['meal_type']}]**")
                        st.caption(rec['recorded_at'][11:16])
                    with c_r2:
                        st.markdown(f"**{rec['food_name']}** — **{rec['calories']} kcal**")
                        st.caption(f"탄수화물 {rec['carbs']}g | 단백질 {rec['protein']}g | 지방 {rec['fat']}g | 당류 {rec['sugar']}g | 나트륨 {rec['sodium']}mg")
                        if rec['feedback']:
                            st.caption(f"💬 *{rec['feedback']}*")
                    with c_r3:
                        if st.button("🗑️ 삭제", key=f"del_rec_{rec['id']}"):
                            if delete_meal_record(rec['id'], user_id):
                                st.success("삭제 완료")
                                st.rerun()
                    st.divider()
        else:
            st.info("해당 일자에 등록된 식단이 없습니다. 사이드바의 '식단 직접 기록하기'를 통해 식단을 등록해보세요!")
